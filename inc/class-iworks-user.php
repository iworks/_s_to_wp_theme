...
class iWorks_User extends iWorks_Theme_Base {
	...
	private function set_fields() {
		$this->fields = apply_filters(
			...
			array(
				...
				'instagram' => __( 'Instagram', 'THEME_SLUG' ),
	...
	public function action_edit_user_profile( $user ) {
		$this->set_fields();
		printf( '<h3 class="heading">%s</h3>', esc_html__( 'Social Media', 'THEME_SLUG' ) );
	...
	public function get_output( $user_id ) {
		...
		foreach ( $this->fields as $key => $label ) {
			...
			$items  .= sprintf(
				...
				esc_url( $value ),
				sprintf(
					esc_html__( '%1$s on %2$s', 'THEME_SLUG' ),
					$user->display_name,
					$label
				)
			);
		...
		$content .= sprintf(
			...
			esc_html__( 'Find me on:', 'THEME_SLUG' )
		);
